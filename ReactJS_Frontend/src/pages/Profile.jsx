...
const Profile = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const updated = { ...user, name: data.user?.name || form.name, avatar: data.user?.avatar || avatarPreview };
            localStorage.setItem("user", JSON.stringify(updated));
            setUser(updated);
            setEditing(false);
            setAvatar(null);
            setMsg("Cập nhật hồ sơ thành công!", "success");
            setMsgType("success");
        } catch (err) {
                ...
                <div className="text-center mb-8">
                    <div className="relative inline-block">
                        <div
                            ...
                            style={{ background: avatarPreview ? "transparent" : "linear-gradient(135deg, #3b82f6, #6366f1)" }}
                        >
                            {avatarPreview ? (
                    ...
                    {editing && (
                        <p className="text-xs text-gray-400 mt-2">Nhấn vào ảnh để thay đổi (tối đa 2MB)</p>
                    )}
                ...
                <div className="space-y-1">
                    {editing ? (
                        ...
                        <>
                            ...
                            <InfoRow icon="calendar" label="Thành viên từ" value={user.createdAt ? new Date(user.createdAt).toLocaleDateString("vi-VN") : "UTEShop"} />
    ...
    return (
        <div className="flex items-center gap-4 py-3 border-b border-gray-100 last:border-0">
            <div className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0">
                <svg className="w-4.5 h-4.5 text-primary w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    {icons[icon]}
                </svg>
            </div>
            <div>
